...
optional.add_argument('--output_path',  help= 'paste path to store output files')
optional.add_argument('--suspend', default = "no", help= 'option to suspend multiprocessing')
optional.add_argument('--cpu_count', default = int(multiprocessing.cpu_count()/2), help= 'option to specify number of CPU for the run')
...
optional.add_argument("--graphical_output", default = "no", help="""If you want to specify graphical output specify -grapical_output yes in the command line followed by --metadata option""")
...
if (args.suspend != "yes") and (args.suspend != "no"):
   sys.exit('\nERROR: --suspend can take only either one of the arguments: "yes" or "no"\n')
    ...
    for line in lines:
     ...
     if (args.species_name != "xen"):
      if (line[0] == 'MT' and line[2]== 'gene'):
        ...
        biotype=line[8].split(";")[-2].split(" ")[-1].replace('"',"")
...
if (args.species_name == "dre") and (args.non_model==None):
   ...
   command = "bowtie-build MT_genome/Danio_rerio.GRCz11.dna.chromosome.MT.fa " + index +  " > ../bowtie_build_log.txt"
   os.system(command)
   ...
   command = "bowtie-build MT_genome/Homo_sapiens.GRCh38.dna.chromosome.MT.fa " + index +  " > ../bowtie_build_log.txt"
   os.system(command)
   print("Downloading gtf annotation file .......")
   ...
   command = "bowtie-build MT_genome/Mus_musculus.GRCm38.dna.chromosome.MT.fa " + index +  " > ../bowtie_build_log.txt"
   os.system(command)
   print("Downloading gtf annotation file .......")
   ...
   command = "bowtie-build MT_genome/Rattus_norvegicus.Rnor_6.0.dna.chromosome.MT.fa " + index +  " > ../bowtie_build_log.txt"
   os.system(command)
   print("Downloading gtf annotation file .......")
   ...
   command = "bowtie-build MT_genome/Gallus_gallus.Gallus_gallus-5.0.dna.chromosome.MT.fa " + index +  " > ../bowtie_build_log.txt"
   os.system(command)
   print("Downloading gtf annotation file .......")
   ...
   if not os.path.exists("MT_genome"):
    os.makedirs("MT_genome")
   os.chdir("MT_genome")
   command = "wget https://ftp.xenbase.org/pub/Genomics/Sequences/Mitochondrial/Xlaevis_mito_seq.fa"
   ...
   command = "bowtie-build MT_genome/Xlaevis_mito_seq.fa " + index +  " > ../bowtie_build_log.txt"
   os.system(command)
   print("Downloading gtf annotation file .......")
   ...
   command = "bowtie-build " + args.FASTA + " " + "non_model/" + index +  " > ../bowtie_build_log.txt"
   os.system(command)
...
if __name__ == "__main__":
 ...
 if (ver == "1.3.0") or (ver == "1.3.1"):
    command = "bowtie --best -v 1 -p " + str(cpu_count) + " -x " + species + "/" + index + " -f master.fa -S filtered200_mt.sam 2> bowtie_log.txt"
    os.system(command)
 else:
    command = "bowtie --best -v 1 -p " + str(cpu_count) + " " + species + "/" + index + " -f master.fa -S filtered200_mt.sam 2> bowtie_log.txt"
    os.system(command)
 ...
 if (len(lines) != 0) and (args.RNA == "sRNA"):
   ...
   if len(lines) > 0:
     #map the sequences to mitochondrial genome index using bowtie
     if (ver == "1.3.0") or (ver == "1.3.1"):
        command = "bowtie --best -v 0 -p " + str(cpu_count) + " -x " + species + "/" + index + " -f CCA_fasta.fa -S filtered200_CCA_mt.sam 2>> bowtie_log.txt"
        os.system(command)
     else:
        command = "bowtie --best -v 0 -p " + str(cpu_count) + " " + species + "/" + index + " -f CCA_fasta.fa -S filtered200_CCA_mt.sam 2>> bowtie_log.txt"
        os.system(command)
     ...
     if len(lines2) != 0:
      ...
      CCA = pd.DataFrame([],columns=["sequence","subtype","type","bio-type", "strand","orientation","annotation","Sequence alignment","Sequence start position(bp)","Sequence end position(bp)","ge\
...
ne-boundary:start(bp)","gene-boundary:end(bp)", "substitutions"])
 ...
 if len(allmtseq) != 0:
  ...
  foy_lst = [tRNA, nontRNA, na_count, CCA]
  add_lst = []
  for x in foy_lst:
   if len(x) != 0:
      add_lst.append(x)

  allmtseq = pd.concat(add_lst)[tRNA.columns.tolist()]
  ...
  allmtseq["temp_e"] = allmtseq["temp_e"].astype(str)
  ...
  os.system("rm -r " + species + " filtered200* master.fa CCA_fasta.fa")
 ...
 if (args.graphical_output == "yes"):
  ...
  for k,v in list(d.items()):
     ...
     df_strand = subset_df.groupby("strand").sum(numeric_only=True)
     df_strand.reset_index(level=0, inplace=True)
     output = k + "_strand.csv"
     df_strand.to_csv(output, sep = ",", index=False)
     df_orientation = subset_df.groupby("orientation").sum(numeric_only=True)
     df_orientation.reset_index(level=0, inplace=True)
     output = k + "_orientation.csv"
     df_orientation.to_csv(output, sep = ",", index=False)
     df_annotation = subset_df.groupby("annotation").sum(numeric_only=True)
     df_annotation.reset_index(level=0, inplace=True)
     output = k + "_annotation.csv"
