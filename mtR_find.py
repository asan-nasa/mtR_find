...
#print list(allmtseq)
 ...
 if (len(lines) != 0) and (args.RNA == "lncRNA"):
   ...
   allmtseq2["Length"] = allmtseq2.apply(lambda x: str(len(x["sequence"])), axis = 1)
   allmtseq2["Specific-ID"]= args.species_name + "-mtlnc" + allmtseq2["type"] + "-" + allmtseq2["Length"].apply(str)+ "-" + allmtseq2["annotation"]+ "-" + allmtseq2["strand"] + "-" + allmtseq2["Sequence start position(bp)"] + allmtseq2["substitutions"]
   allmtseq2["General-ID"]= args.species_name + "-mtlnc" + allmtseq2["type"] + "-"  + allmtseq2["annotation"]+ "-" + allmtseq2["strand"] + "-" + allmtseq2["Sequence start position(bp)"] 
   ...
   #asan.to_csv("lnc_RNA2.csv", sep = ",", index=False)
 ...
 if (args.graphical_output == "yes"):
  ...
  for k,v in list(d.items()):
     merged_list = []
     #v = [ item + "_trimmed" for item in v]
     v = [ item.split("_trimmed")[0] for item in v]
